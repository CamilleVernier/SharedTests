...
*/
		...
		if(cmp_nocase_no__(var,"Ploidy")==0) {
			...
			stringstream strstr(buf.substr(pos+1));
			strstr>>locstring;
			if(cmp_nocase_no__(locstring,"Diploid")==0 || cmp_nocase_no__(locstring,"Diploïd")==0) ploidy=2;
			else if(cmp_nocase_no__(locstring,"Haploid")==0 || cmp_nocase_no__(locstring,"Haploïd")==0) ploidy=1;
        ...
        if(cmp_nocase_no__(var,"MIGfile")==0) {
            evaluateBool(migFilebool,buf.substr(pos+1));
            goto nextline;
        }
        if(cmp_nocase_no__(var,"GRAfile")==0) {
            evaluateBool(graFilebool,buf.substr(pos+1));
            goto nextline;
        }
		...
		if(cmp_nocase_no__(var,"DiagnosticTables")==0 || cmp_nocase_no__(var,"DiagnosticTable")==0) { // FR->RL new mechanism to pass options to R in a more economical way that all the booleans in Roptions
            ...
            while (buf.length()>0) {
               ...
               else if (cmp_nocase_no__(arg,"noHeader")==0) {noHeader=true;} //
               else if (cmp_nocase_no__(arg,"arRegression")==0) {arRegression=true;iterativeStats=true;} //
               else if (cmp_nocase_no__(arg,"erRegression")==0) {erRegression=true;iterativeStats=true;} //
               else if (cmp_nocase_no__(arg,"moranIRegression")==0 || cmp_nocase_no__(arg,"iRegression")==0) {moranIRegression=true;iterativeStats=true;} //
        ...
        if(cmp_nocase_no__(var,"minDist")==0 || cmp_nocase_no__(var,"RegMinDist")==0 || cmp_nocase_no__(var,"MinDistReg")==0) {
            stringstream strstr(buf.substr(pos+1));
            strstr>>NS_diagnostic_tables::minDistReg;
            goto nextline;
        }
		...
		if(cmp_nocase_no__(var,"Group_All_Samples")==0 || cmp_nocase_no__(var,"Group_Samples")==0 || cmp_nocase_no__(var,"Group_Sample")==0) {
			evaluateBool(groupAllSamplesbool,buf.substr(pos+1));
			goto nextline;
		}
        if(cmp_nocase_no__(var,"Individualize_All_Samples")==0 || cmp_nocase_no__(var,"Individualize_Samples")==0 || cmp_nocase_no__(var,"Individualize_Sample")==0) {
            evaluateBool(individualizeAllSamplesbool,buf.substr(pos+1));
            goto nextline;
        }
		...
		if(cmp_nocase_no__(var,"Barrier")==0 || cmp_nocase_no__(var,"BackwardBarrier")==0) {
            evaluateBool(TVpars.back().backwardBarrier,buf.substr(pos+1));
			goto nextline;
		}
		...
		if(cmp_nocase_no__(var,"BarrierCrossingRate")==0 || cmp_nocase_no__(var,"CrossingRate")==0) {
			stringstream strstr(buf.substr(pos+1));
			strstr>>TVpars.back().barrierCrossingRateUp;
            TVpars.back().barrierCrossingRateDown=TVpars.back().barrierCrossingRateUp;
			goto nextline;
		}		
        if(cmp_nocase_no__(var,"BarrierCrossingRateUp")==0 || cmp_nocase_no__(var,"CrossingRateUp")==0) {
            stringstream strstr(buf.substr(pos+1));
            strstr>>TVpars.back().barrierCrossingRateUp;
            goto nextline;
        }		
        if(cmp_nocase_no__(var,"BarrierCrossingRateDown")==0 || cmp_nocase_no__(var,"CrossingRateDown")==0) {
            stringstream strstr(buf.substr(pos+1));
            strstr>>TVpars.back().barrierCrossingRateDown;
            goto nextline;
        }		
...
//    ***********************************************************************
    ...
    if(migraineSettingsbool && nMarker>1) {
            cerr << endl;cout << endl;cout << endl;cout << endl;cout << endl;
            cerr << "nMarker=" << nMarker << endl;
    ...
    if (groupAllSamplesbool && individualizeAllSamplesbool) {
        cerr << endl;cout << endl;cout << endl;cout << endl;cout << endl;
        cerr << "Contradiction in settings file between Individualize_All_Samples=T and Group_All_Samples=T" << endl;
        cerr << "Check settings. I exit." << endl;
        if (cinGetOnError)
            cin.get();
        exit(-1);
    ...
    **/
	...
	while (it!=TVpars.end()) {
		...
		if ((it->dimRes1 % it->vide) !=0 || (it->dimRes2 % it->vide) != 0) {
            cerr<<"\n In demographic Phase "<<locit<<" at " << it->minGen << " generations backward in time :";
            cerr<<"\n Any of lattice_size_X or lattice_size_Y is not a multiple or void_nodes. I exit";
            if (cinGetOnError) cin.get();
            exit(-1);
        }
		if(it->backwardBarrier || it->forwardBarrier) {
			...
			if ( (it->barrierCrossingRateUp > 1.0) || (it->barrierCrossingRateDown > 1.0) ) {
				cerr<<"\n In demographic Phase "<<locit<<" at " << it->minGen << " generations backward in time :";
				if (it->barrierCrossingRateUp==it->barrierCrossingRateDown) cerr<<"\n (!) barrierCrossingRate is > 1.0 (!) . Check your settings. I exit."<< endl;
                else cerr<<"\n (!) One of the two asymetrical barrierCrossingRateUp ot Down is > 1.0 (!) . Check your settings. I exit."<< endl;
				if (cinGetOnError) cin.get();
				exit(-1);
			}
			if ((it->y1_barrier != it->y2_barrier) && (it->x1_barrier != it->x2_barrier)) {
				cerr<<"\n In demographic Phase "<<locit<<" at " << it->minGen << " generations backward in time :";
				cerr<<"\n The barrier is not a horizontal or vertical line. The current version of IBDSim can not consider such barriers. I exit";
				if (cinGetOnError) cin.get();
				exit(-1);
			}
			if ((it->y1_barrier == it->y2_barrier) && (it->x1_barrier > it->x2_barrier)) {
			...
			if ( (it->dimRes2 != 1) && ( ( (it->x1_barrier == it->x2_barrier) && (it->y1_barrier == it->y2_barrier) ) || ( (it->barrierCrossingRateUp > 0.99999) && (it->barrierCrossingRateDown > 0.99999) ) ) ) {
				cerr<<"\n In demographic Phase "<<locit<<" at " << it->minGen << " generations backward in time :";
            ...
            if ( (it->dimRes2 == 1) && ( (it->y1_barrier != it->y2_barrier) && (it->y1_barrier != 1) ) ) {
                cerr<<"\n In demographic Phase "<<locit<<" at " << it->minGen << " generations backward in time :";
                cerr<<"\n Barrier settings are not compatible with a 1D lattice/habitat ";
                cerr<<"\n because at least one of the following conditions is violated : ";
                cerr << "y1=y2=1 and x1=x2; in settings file " << SettingsFilename<< endl;
                cerr << "I exit. "<< endl;
                if (cinGetOnError) cin.get();
                exit(-1);
            }
            if ( (it->dimRes2 == 1) && ( (it->barrierCrossingRateUp > 0.99999) && (it->barrierCrossingRateDown > 0.99999) ) ) {
                cerr<<"\n In demographic Phase "<<locit<<" at " << it->minGen << " generations backward in time :";
                cerr<<"\n Barrier settings will not have any effect because";
                cerr << "barrierCrossingRate > 0.99999 in settings file " << SettingsFilename<< endl;
                cerr << "I exit. " << SettingsFilename<< endl;
                if (cinGetOnError) cin.get();
                exit(-1);
            }
			if ( ( (it->barrierCrossingRateUp < 0.0000000001) && (it->barrierCrossingRateDown < 0.0000000001) ) && ( ((it->x1_barrier == 1) && (it->x2_barrier == it->dimRes1)) || ((it->y1_barrier == 1) && (it->y2_barrier == it->dimRes2)) ) ) {
                int FoundIncompleteBarrier=false;
                vector<CTimeVaryingParams>::iterator itloc=(it+1);
                while (itloc!=TVpars.end()) {
                    if( (itloc->backwardBarrier || itloc->forwardBarrier)
                        && ! ( (it->barrierCrossingRateUp < 0.0000000001) && (it->barrierCrossingRateDown < 0.0000000001) ) && ( ((itloc->x1_barrier == 1) && (itloc->x2_barrier == itloc->dimRes1)) || ((itloc->y1_barrier == 1) && (itloc->y2_barrier == itloc->dimRes2)) )) FoundIncompleteBarrier=true;
                        else if(!itloc->backwardBarrier && !itloc->forwardBarrier) FoundIncompleteBarrier=true;
                    itloc++;
                }
                if(!FoundIncompleteBarrier) {
                    cerr<<"\n In demographic Phase "<<locit<<" at " << it->minGen << " generations backward in time :";
                    if(it->barrierCrossingRateUp == it->barrierCrossingRateDown) cerr<<"\n (!) barrierCrossingRate is " << it->barrierCrossingRateUp << " (< 0.0000000001) with a complete vertical or horizontal barrier (!) ." << endl;
                    else cerr<<"\n (!) Both asymetric barrierCrossingRate " << it->barrierCrossingRateUp << " and " << it->barrierCrossingRateDown << " are < 0.0000000001, with a complete vertical or horizontal barrier (!) ." << endl;
            ...
            if (it->dimRes1>1 && (it->x1_barrier == it->x2_barrier) ) {
             if(it->backwardBarrier) it->backwardBarrierX=true; else it->forwardBarrierX=true;
            }
            if (it->dimRes2>1 && (it->y1_barrier == it->y2_barrier) ) {
             if(it->backwardBarrier) it->backwardBarrierY=true; else it->forwardBarrierY=true;   
            }
		}
	...
	if(!Simple1DProductDispBool) {//RL072014 was cmp_nocase(twoD_disp,"1DProductWithoutm0")==0) {
        ...
        while (it!=TVpars.end()) {
            if (it->mod != 'g' || it->mod != 'S') {
                cerr<<"\n twoD_disp==\"1DProductWithoutm0\", with incompatible dispersal model (not geometric or Sichel) in demographic Phase "<<locit<<" at " << it->minGen << " generations backward in time :";
